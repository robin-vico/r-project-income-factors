...
```{r}
ggplot(over_50k) +
    geom_histogram(aes(x = over_50k$education.num))
```
...
a = over_50k %>%
  count(education.num) %>%
  arrange(desc(n)) %>%
  mutate(percentage = n*100/7650)
print(a)
```

```{r}
ggplot(a) +
    geom_histogram(aes(x = a$percentage))
```
...
```{r}
sample_0 = final_data %>%
  filter(income.simplified == FALSE) %>%
...
```{r}
ggplot(sample_0) +
    geom_histogram(aes(x = hours.per.week))
